...
phi = 0%CameraHeadData;                                   % Angle of object from center, DEGREES
convFLG = 0
...
ang_cone1 = theta +35           %probability cone from camera: upper bound angle WRT x-axis
ang_cone0 = theta - 35          %lower bound WRT x-axis
...
while 1
    ...
    for n=ang_cone0:ang_cone1  
        ...
        if occupied                                 % breaks while loop if condition is met
            ...
            if(abs(yaw-phi) < 10)
                OBJ( xEst,yEst) =  2%CameraColorSub;
            break
            else
                break
            end   
