...
function [u,up,ui,ud,error,int] = calculate_gains(k,time_params,pid_gains,u,up,ui,ud,error,int,behavior)
    ...
    elseif behavior(1,k) == 9        % control psi to goal heading and drive straight at set duty cycle
        ...
        % Proportional gain       
        up(1,k) = pid_gains(4)*error(12,k)+pid_gains(7)*error(6,k)-pid_gains(7)*error(11,k);
        up(2,k) = -pid_gains(4)*error(12,k)+pid_gains(7)*error(6,k)+pid_gains(7)*error(11,k);
        up(3,k) = -pid_gains(4)*error(12,k)+pid_gains(7)*error(6,k)-pid_gains(7)*error(11,k);
        up(4,k) = pid_gains(4)*error(12,k)+pid_gains(7)*error(6,k)+pid_gains(7)*error(11,k);
        % Integral gain
        if k > 1
            int = increment_integrator(int,k,error,time_params);
            ui(1,k) = pid_gains(5)*int(12,k)+pid_gains(8)*int(6,k)-pid_gains(8)*int(11,k);
            ui(2,k) = -pid_gains(5)*int(12,k)+pid_gains(8)*int(6,k)+pid_gains(8)*int(11,k);
            ui(3,k) = -pid_gains(5)*int(12,k)+pid_gains(8)*int(6,k)-pid_gains(8)*int(11,k);
            ui(4,k) = pid_gains(5)*int(12,k)+pid_gains(8)*int(6,k)+pid_gains(8)*int(11,k);
        end
        % Derivative gain
        if k > 1
            ud(1,k) = pid_gains(6)*((error(12,k)-error(12,k-1))/time_params(1))+pid_gains(9)*((error(6,k)-error(6,k-1))/time_params(1))-pid_gains(9)*((error(11,k)-error(11,k-1))/time_params(1));
            ud(2,k) = -pid_gains(6)*((error(12,k)-error(12,k-1))/time_params(1))+pid_gains(9)*((error(6,k)-error(6,k-1))/time_params(1))+pid_gains(9)*((error(11,k)-error(11,k-1))/time_params(1));
            ud(3,k) = -pid_gains(6)*((error(12,k)-error(12,k-1))/time_params(1))+pid_gains(9)*((error(6,k)-error(6,k-1))/time_params(1))-pid_gains(9)*((error(11,k)-error(11,k-1))/time_params(1));
            ud(4,k) = pid_gains(6)*((error(12,k)-error(12,k-1))/time_params(1))+pid_gains(9)*((error(6,k)-error(6,k-1))/time_params(1))+pid_gains(9)*((error(11,k)-error(11,k-1))/time_params(1));
        end
        limit_mode = 2;
    end
